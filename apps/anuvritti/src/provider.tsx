...
export function AnuvrittiProvider({ children, fallback = null }: AnuvrittiProviderProps) {
   ...
   * in the product. `clear()` empties the token store and nothing else.
   */
  const revoked = useCallback(() => {
    setToken(null);
    setPaired(false);
    void wiredRef.current?.tokens.clear();
  }, []);

  useEffect(() => {
    let cancelled = false;
    void wire(DEFAULT_BASE_URL, { onRevoked: () => revoked() }).then(async (ready) => {
